...
| Monthly Progress | Pass | `src/features/progress/ProgressScreen.tsx` includes `Monthly Progress`, compact month-end snapshot status, no-snapshot/insufficient-data states, portfolio vs invested chart, assets vs months chart, asset class snapshot, recent snapshots, and review-gated snapshot edit fields. | Updated for issue #125: the manual snapshot form is no longer the default surface; generated/review status is the primary V1 behavior. |
...
| No monthly snapshots | Pass | `src/features/progress/__tests__/ProgressScreen.test.tsx` covers the no-snapshot state, compact month-end status, and review-gated snapshot editing. | Updated for issue #125 smart month-end snapshot behavior. |
