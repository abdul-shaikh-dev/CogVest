...
| Add Holding opening-position flow | `src/features/openingPositions/__tests__/AddOpeningPositionForm.test.tsx`, `src/features/openingPositions/__tests__/useAddOpeningPosition.test.tsx`, `src/testing/__tests__/e2eEvidence.test.ts` | `e2e/add-holding-lookup.yaml`, `e2e/add-holding-manual-semantics.yaml`, and `e2e/add-holding-edited-quote.yaml` assert persisted lookup/manual paths | Read-only evidence asserts identity, class, currency, exchange/provider ID, quote provenance, invested/current values, position count, and zero canonical conflicts; success copy alone is insufficient |
