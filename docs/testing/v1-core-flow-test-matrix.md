...
| Protected detailed CAS | Parser, normalization, planner and blocked-screen tests retain fail-closed row/balance checks | `e2e/cas-native-import.yaml` uses a wholly synthetic protected two-page PDF through the Android reader, wrong-password retry, a value-free cancellation notice in review, persisted balances and repeat import; requires fresh debug APK + Metro | 2 imported trades, 12 units, INR 1,200 invested, unchanged cash/opening counts, 2 duplicates on retry; numeric cancellations and mismatches remain blocked |
