...
import { isAutoGeneratedSnapshotProvisional } from "./monthEndSnapshots";
import {
  decimal,
  normalizeMoney,
  normalizePercentage,
...
export function buildMonthlyProgressChartData(
  ...
  const estimatedIndices = filtered.flatMap((snapshot, index) =>
    isAutoGeneratedSnapshotProvisional(snapshot) ? [index] : [],
  );
  const gapAfterIndices = filtered.flatMap((snapshot, index) => {
    const next = filtered[index + 1];
    if (!next) return [];
    const [year, month] = snapshot.month.split("-").map(Number);
    const expected = new Date(Date.UTC(year, month, 1)).toISOString().slice(0, 7);
    return next.month === expected ? [] : [index];
  });
